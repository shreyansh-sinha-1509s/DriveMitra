...
async function loadDashboard() {
  ...
  try {
    ...
    const feedbackRes = await fetch(`${API_BASE}/feedback/driver/${encodeURIComponent(vehicleNo)}`);
    ...
    const recentSlips = document.getElementById("recentSlips");

    if (totalSlips) totalSlips.textContent = Array.isArray(slips) ? slips.length : 0;

    if (totalFeedbackCount) totalFeedbackCount.textContent = Array.isArray(feedbacks) ? feedbacks.length : 0;

    if (recentSlips) {
      if (Array.isArray(slips) && slips.length > 0) {
        recentSlips.innerHTML = slips
          .slice(0, 5)
          .map(
            (s) => `
              <div class="feedback-item" style="border-left: 4px solid #3b82f6; padding-left: 12px; margin-bottom: 8px;">
                <strong>Slip ID: ${s.slipId}</strong><br>
                <small><strong>Customer:</strong> ${s.customerName} | <strong>Location:</strong> ${s.location}</small><br>
                <small><strong>Item:</strong> ${s.itemType} (Qty: ${s.quantity})</small><br>
                <small style="color:#6b7280">${new Date(s.createdAt).toLocaleString()}</small>
              </div>
            `
          )
          .join("");
      } else {
        recentSlips.textContent = "No recent deliveries found";
      }
    }
    ...
    if (recentFeedback) {
      if (Array.isArray(feedbacks) && feedbacks.length > 0) {
        recentFeedback.innerHTML = feedbacks
          ...
          .map(
            (f) => `
              <div class="feedback-item" style="border-left: 4px solid #f59e0b; padding-left: 12px; margin-bottom: 8px;">
                <strong>${f.customer_name}</strong> - ${f.rating}⭐<br>
                <small>${f.item_type}, Qty: ${f.quantity}</small><br>
